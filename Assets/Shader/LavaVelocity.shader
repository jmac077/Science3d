Shader "Erosion/LavaVelocity" 
{
	SubShader 
	{
    	Pass 
    	{
			...
			uniform sampler2D _LavaField, _LavaFieldOld, _OutFlowField;
			...
			float4 frag(v2f IN) : COLOR
			{
				...
				float waterC = tex2D(_LavaField, IN.uv).x;
				float waterCOld = tex2D(_LavaFieldOld, IN.uv).x;
